...
test.describe('dashboard render', () => {
  ...
  test('s5 renders unified wealth model rows and assumptions', async ({ page }) => {
    ...
    await expect(page.locator('#kpi_wealth')).not.toContainText(/\$\d+(?:\.\d+)?M/);
    await expect(page.locator('#wealth_table_card')).not.toContainText(/\$\d+(?:\.\d+)?K/);
    await expect(page.locator('#wealth_table_card')).not.toContainText(/\$\d+(?:\.\d+)?M/);
    await expect(page.locator('#s5')).not.toContainText(/\$\d+(?:\.\d+)?K/);
    await expect(page.locator('#s5')).not.toContainText(/\$\d+(?:\.\d+)?M/);
