...
test.describe('dashboard render', () => {
  ...
  test('s5 renders unified wealth model rows and assumptions', async ({ page }) => {
    ...
    await expect(page.locator('#assumptions_box')).toBeHidden();
    await page.locator('summary:has-text("默认假设与口径")').click();
    await expect(page.locator('#assumptions_box')).toBeVisible();
    ...
    await expect(page.locator('#i_female_oa')).toHaveValue('8000');
    await expect(page.locator('#i_tax_giro')).toHaveValue('700');
    await expect(page.locator('#i_rsu_after_tax')).toHaveValue('1100');
    await expect(page.locator('#i_total_yrs')).toHaveValue('5');
    await expect(page.locator('#assumptions_box')).toContainText('税款 / RSU 现金流');
    await expect(page.locator('#wealth_table_card')).toContainText('前期总资金支出');
    await expect(page.locator('#wealth_table_card .metric-sub').first()).toContainText('现金支出');
    await expect(page.locator('#wealth_table_card .metric-sub').nth(1)).toContainText('CPF OA 支出');
    await expect(page.locator('#wealth_table_card .metric-sub2')).toContainText('其中 OA 不足转现金');
    await expect(page.locator('#wealth_table_card')).toContainText('卖房成本');
    await expect(page.locator('#wealth_table_card')).toContainText('总财富 CAGR');
    await expect(page.locator('#wealth_table_card')).toContainText('起始80万');
    await expect(page.locator('#wealth_table_card')).toContainText('月住房现金支出(Y1-2，合计)');
    await expect(page.locator('#wealth_table_card')).toContainText('其中：房贷现金(Y1-2)');
    await expect(page.locator('#wealth_table_card')).toContainText('其中：MCST/maintenance(Y1-2)');
    await expect(page.locator('#wealth_table_card')).toContainText('卖房成本（合计）');
    await expect(page.locator('#wealth_table_card')).toContainText('其中：卖方中介费');
    ...
    await expect(page.locator('#lg_2b2b_explain')).toContainText('前期 OA8000');
    await expect(page.locator('#kpi_wealth')).not.toContainText(/\$\d+(?:\.\d+)?K/);
    await expect(page.locator('#wealth_table_card')).not.toContainText(/\$\d+(?:\.\d+)?K/);
    await expect(page.locator('#s5')).not.toContainText(/\$\d+(?:\.\d+)?K/);
    await expect(page.locator('#monthly_detail_table')).toContainText('OA抵扣(Y1-');
    await expect(page.locator('#monthly_detail_table')).toContainText('MCST/maintenance');
    await expect(page.locator('#monthly_detail_table')).toContainText('$330');
